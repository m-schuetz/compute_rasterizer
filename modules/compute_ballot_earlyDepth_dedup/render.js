...
function render_compute_ballot_earlyDepth_dedup(node, view, proj, target){
	...
	if(typeof computeState_ballot_earlyDepth_dedup === "undefined"){

		let pathRender = `${rootDir}/modules/compute_ballot_earlyDepth_dedup/render.cs`;
		let pathResolve = `${rootDir}/modules/compute_ballot_earlyDepth_dedup/resolve.cs`;
		...
		let ssboDebug = gl.createBuffer();
		gl.namedBufferData(ssboDebug, 8, new ArrayBuffer(4), gl.DYNAMIC_DRAW);

		let fbo = new Framebuffer();

		computeState_ballot_earlyDepth_dedup = {
			csRender, csResolve, numPixels, ssboFramebuffer, fbo, ssboDebug
		};
	}

	let csRender = computeState_ballot_earlyDepth_dedup.csRender;
	let csResolve = computeState_ballot_earlyDepth_dedup.csResolve;
	let ssboFramebuffer = computeState_ballot_earlyDepth_dedup.ssboFramebuffer;
	let ssboDebug = computeState_ballot_earlyDepth_dedup.ssboDebug;
	let fbo = computeState_ballot_earlyDepth_dedup.fbo;
	...
	if(true){ // RENDER
		...
		gl.bindBufferBase(gl.SHADER_STORAGE_BUFFER, 2, ssboDebug);
	...
	// if((frameCount % 100) === 0){
	// 	printSSBO(ssboDebug, Uint32Array, 0, 2);
	// }

	// {
	// 	gl.memoryBarrier(gl.ALL_BARRIER_BITS);
	// 	// gl.clearNamedBufferData(ssboDebug, gl.R32F, gl.UNSIGNED_INT, gl.UNSIGNED_INT, new Uint32Array([0]));
	// 	let cRGB = new Uint32Array([0, 0]);
	// 	gl.clearNamedBufferData(ssboDebug, gl.R32UI, gl.RED_INTEGER, gl.UNSIGNED_INT, cRGB);
	// 	gl.memoryBarrier(gl.ALL_BARRIER_BITS);
	// }
