...
function render_compute_ballot_earlyDepth_supersampling(node, view, proj, target){
	...
	let width = 1920;
	let height = 1080;
	let supersampleSize = 1;

	if(typeof computeState_ballot_earlyDepth_supersampling === "undefined"){

		let pathRender = `${rootDir}/modules/compute_ballot_earlyDepth_supersampling/render.cs`;
		let pathResolve = `${rootDir}/modules/compute_ballot_earlyDepth_supersampling/resolve.cs`;
		...
		let numPixels = 1.5 * supersampleSize * supersampleSize * width * height; // TODO support resizing
		...
		computeState_ballot_earlyDepth_supersampling = {
			csRender, csResolve,
			numPixels, ssboFramebuffer, fbo,
		};
	}

	let csRender = computeState_ballot_earlyDepth_supersampling.csRender;
	let csResolve = computeState_ballot_earlyDepth_supersampling.csResolve;
	let ssboFramebuffer = computeState_ballot_earlyDepth_supersampling.ssboFramebuffer;
	let fbo = computeState_ballot_earlyDepth_supersampling.fbo;
	...
	{ // RENDER
		...
		for(let buffer of node.glBuffers){
			...
			gl.uniform2i(csRender.uniforms.uImageSize, supersampleSize * width, supersampleSize * height);
	...
	{ // RESOLVE
		...
		if(csResolve.uniforms.uSupersampleSize){
			gl.uniform1i(csResolve.uniforms.uSupersampleSize, supersampleSize);
		}
		...
		let cRGB = new Uint32Array([0xffffffff]);
		gl.clearNamedBufferData(ssboFramebuffer, gl.R32UI, gl.RED_INTEGER, gl.UNSIGNED_INT, cRGB);
