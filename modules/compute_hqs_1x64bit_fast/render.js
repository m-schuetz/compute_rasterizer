...
function renderComputeHQS_1x64bit_fast(node, view, proj, target){
	...
	if(typeof computeHQS_1x64State_fast === "undefined"){

		let pathRenderDepth = `${rootDir}/modules/compute_hqs_1x64bit_fast/render_depth.cs`;
		let pathRenderAttribute = `${rootDir}/modules/compute_hqs_1x64bit_fast/render_attribute.cs`;
		let pathResolve = `${rootDir}/modules/compute_hqs_1x64bit_fast/resolve.cs`;
		...
		gl.namedBufferData(ssBA, numPixels * 16, 0, gl.DYNAMIC_DRAW);

		let fbo = new Framebuffer();

		computeHQS_1x64State_fast = {
	...
	let {csRenderDepth, csRenderAttribute, csResolve} = computeHQS_1x64State_fast;
	let {ssboDepthbuffer, ssboFramebuffer, ssRG, ssBA} = computeHQS_1x64State_fast;
	let {fbo} = computeHQS_1x64State_fast;
