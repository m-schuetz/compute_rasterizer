...
function render_compute_ballot(node, view, proj, target){

	GLTimestamp("compute-ballot-start");

	// TODO support resizing
	let width = 3000;
	let height = 2000;

	if(typeof computeState_ballot === "undefined"){

		let pathRender = `${rootDir}/modules/compute_ballot/render.cs`;
		let pathResolve = `${rootDir}/modules/compute_ballot/resolve.cs`;
	...
	let csRender = computeState_ballot.csRender;
	let csResolve = computeState_ballot.csResolve;
	let ssboFramebuffer = computeState_ballot.ssboFramebuffer;
	let fbo = computeState_ballot.fbo;
	...
	if(true){ // RENDER
		...
		GLTimestamp("compute-ballot-render-start");
		...
		gl.useProgram(0);
		GLTimestamp("compute-ballot-render-end");
	}

	gl.memoryBarrier(gl.ALL_BARRIER_BITS);

	{ // RESOLVE
		GLTimestamp("compute-ballot-resolve-start");
		...
		gl.useProgram(0);
		GLTimestamp("compute-ballot-resolve-end");
	}
	...
	GLTimestamp("compute-ballot-end");
