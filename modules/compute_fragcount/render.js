...
function render_compute_fragcount(node, view, proj, target){

	GLTimestamp("compute-fragcount-start");

	// TODO support resizing
	let width = 3000;
	let height = 2000;

	if(typeof computeState_fragcount === "undefined"){

		let pathRender = `${rootDir}/modules/compute_fragcount/render.cs`;
		let pathResolve = `${rootDir}/modules/compute_fragcount/resolve.cs`;
	...
	let csRender = computeState_fragcount.csRender;
	let csResolve = computeState_fragcount.csResolve;
	let ssboFramebuffer = computeState_fragcount.ssboFramebuffer;
	let fbo = computeState_fragcount.fbo;
	...
	if(true){ // RENDER
		GLTimestamp("compute-fragcount-render-start");
		...
		gl.useProgram(0);
		GLTimestamp("compute-fragcount-render-end");
	}

	gl.memoryBarrier(gl.ALL_BARRIER_BITS);

	{ // RESOLVE
		GLTimestamp("compute-fragcount-resolve-start");
		...
		gl.useProgram(0);
		GLTimestamp("compute-fragcount-resolve-end");
	}
	...
	GLTimestamp("compute-fragcount-end");
