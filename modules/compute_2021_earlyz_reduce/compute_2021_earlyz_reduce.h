...
struct ComputeEarlyZReduce : public Method{
	...
	ComputeEarlyZReduce(Renderer* renderer, shared_ptr<LasStandardData> las){

		this->name = "(2021) early-z & reduce";
		this->description = R"ER01(
		)ER01";
		this->las = las;
		this->group = "2021 method; standard 16 byte per point";

		csRender = new Shader({ {"./modules/compute_2021_earlyz_reduce/render.cs", GL_COMPUTE_SHADER} });
		csResolve = new Shader({ {"./modules/compute_2021_earlyz_reduce/resolve.cs", GL_COMPUTE_SHADER} });
