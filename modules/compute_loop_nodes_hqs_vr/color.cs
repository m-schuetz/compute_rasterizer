...
layout (std430, binding =  2) buffer abc_1 { uint64_t ssLeft_rgba[]; };
layout (std430, binding =  3) buffer abc_2 { uint32_t ssRight_depth[]; };
layout (std430, binding =  4) buffer abc_3 { uint64_t ssRight_rgba[]; };
...
layout (std430, binding = 44) buffer abc_8 { uint32_t ssRGBA[]; };
...
void main(){
	...
	for(int i = 0; i < loopSize; i++){
		...
		// float w = getWeight(pos.xyz);
		// R = uint(255 * w);
		// G = 0;
		// B = 0;

		// average points within 1% distance
		float avgDistanceFactor = 1.01;
		...
		if(leftInFrustum){
			...
			bool visible = (depth <= bufferDepth * avgDistanceFactor);

			if(visible){
				// int64_t RG = (int64_t(R) << 0) | (int64_t(G) << 32);
				// int64_t BA = (int64_t(B) << 0) | (int64_t(1) << 32);
				...
				atomicAdd(ssLeft_rgba[pixelID ], RGBA);
		...
		if(rightInFrustum){
			...
			if(visible){
				...
				atomicAdd(ssRight_rgba[pixelID ], RGBA);
