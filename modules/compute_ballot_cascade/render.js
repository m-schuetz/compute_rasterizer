...
function render_compute_ballot_cascade(node, view, proj, target){

	GLTimestamp("compute-ballot-cascade-start");

	// TODO support resizing
	let width = 3000;
	let height = 2000;

	if(typeof computeState_ballot_cascade === "undefined"){

		let pathRender = `${rootDir}/modules/compute_ballot_cascade/render.cs`;
		let pathResolve = `${rootDir}/modules/compute_ballot_cascade/resolve.cs`;
	...
	let csRender = computeState_ballot_cascade.csRender;
	let csResolve = computeState_ballot_cascade.csResolve;
	let ssboFramebuffer = computeState_ballot_cascade.ssboFramebuffer;
	let fbo = computeState_ballot_cascade.fbo;
	...
	if(true){ // RENDER
		...
		GLTimestamp("compute-ballot-cascade-render-start");
		...
		gl.useProgram(0);
		GLTimestamp("compute-ballot-cascade-render-end");
	}

	{ // RESOLVE
		GLTimestamp("compute-ballot-cascade-resolve-start");
		...
		gl.useProgram(0);
		GLTimestamp("compute-ballot-cascade-resolve-end");
	}
	...
	GLTimestamp("compute-ballot-cascade-end");
