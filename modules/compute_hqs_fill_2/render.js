...
function renderComputeHQS_fill_2(node, view, proj, target){

	// TODO support resizing
	let width = 3000;
	let height = 2000;

	if(typeof computeHQSFill_2_state === "undefined"){

		let pathRenderDepth = `${rootDir}/modules/compute_hqs_fill_2/render_depth.cs`;
		let pathRenderAttribute = `${rootDir}/modules/compute_hqs_fill_2/render_attribute.cs`;
		let pathResolve = `${rootDir}/modules/compute_hqs_fill_2/resolve.cs`;
		...
		let vsPath = `${rootDir}/modules/compute_hqs_fill_2/fill.vs`;
		let fsPath = `${rootDir}/modules/compute_hqs_fill_2/fill.fs`;
	...
	let {csRenderDepth, csRenderAttribute, csResolve} = computeHQSFill_2_state;
	let {ssboDepthbuffer, ssRGBA} = computeHQSFill_2_state;
	let {fbo, fboDilated, texDepth} = computeHQSFill_2_state;

	GLTimestamp("compute-hqs_fill2-start");
	...
	{ // RENDER DEPTH
		...
		GLTimestamp("compute-hqs_fill2-depth-start");
		...
		gl.useProgram(0);
		GLTimestamp("compute-hqs_fill2-depth-end");
	}

	{ // RENDER ATTRIBUTE

		GLTimestamp("compute-hqs_fill2-attribute-start");
		...
		gl.useProgram(0);
		GLTimestamp("compute-hqs_fill2-attribute-end");
	}

	{ // RESOLVE
		GLTimestamp("compute-hqs_fill2-resolve-start");
		...
		gl.useProgram(0);
		GLTimestamp("compute-hqs_fill2-resolve-end");
	}

	{ // DILATE
		GLTimestamp("compute-hqs_fill2-fill-start");

		let shader = computeHQSFill_2_state.fillShader;
		...
		GLTimestamp("compute-hqs_fill2-fill-end");
	}
	...
	GLTimestamp("compute-hqs_fill2-end");
